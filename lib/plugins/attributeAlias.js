...
function applyAlias(mappings) {
    return function aliasApplier(attribute) {
        ...
        if (config.checkNgsi2() && typeof(attribute.value) === 'string') {
            if (attribute.type === 'Integer') {
                attribute.value = Number.parseInt(attribute.value);
            }
            else if (attribute.type === 'Float') {
                attribute.value = Number.parseFloat(attribute.value);
            }
            else if (attribute.type === 'Boolean') {
                attribute.value = (attribute.value === 'true' || attribute.value === '1');
            }
            else if (attribute.type === 'None') {
                attribute.value = null;
            }
            else if (attribute.type === 'Array' || attribute.type === 'Object') {
                try {
                    var parsedValue = JSON.parse(attribute.value);
                    attribute.value = parsedValue;
                } catch (e) {
                    logger.error(context, 'Bad attribute value type. Expecting JSON Array or JSON Object. Received:%s',attribute.value);
                }
            }
        }
