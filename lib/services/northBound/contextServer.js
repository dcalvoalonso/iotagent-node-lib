...
function handleQueryNgsi2(req, res, next) {
    ...
    // Heuristic: we are assuming that the entities array in POST /v2/op/query body has only one item

    // FIXME: If the entity has idPattern: '.*' it refers to a whole type (i.e. contextEntity.id is not defined)
    // This is not very clean... could probably be improved
    if (!(req.body.entities[0].idPattern && req.body.entities[0].idPattern === '.*' ))
    {
