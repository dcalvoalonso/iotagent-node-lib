...
var async = require('async'),
    ...
    updateContextTemplateNgsi2 = require('../../templates/updateContextNgsi2.json'),
...
function handleUpdateNgsi2(req, res, next) {
    ...
    if (updateHandler || commandHandler) {
        ...
        async.waterfall([
            apply(async.map, req.body.entities, apply(generateUpdateActionsNgsi2, req)),
            reduceActions,
            async.series
...
function handleQueryNgsi2(req, res, next) {
    ...
    function createQueryRequest(attributes, contextEntity, callback) {
        ...
        getFunction(function handleFindDevice(error, innerDevice) {
            ...
            async.map(deviceList, async.apply(
                finishQueryForDevice, attributes, contextEntity, actualHandler), function (error, results) {
                if (error) {
                    callback(error);
                }
    ...
    {
        logger.warn('queries with entities number different to 1 are not supported (%d found)',
            req.body.entities.length);
        handleQueryContextRequests({code: 400, name: 'NotSupported', message: 'more than one entity in query'});
        return;
    }
    if (req.body.entities[0].idPattern)
    {
        logger.warn('queries with idPattern number different to 1 are not supported (%d found)',
            req.body.entities.length);
        handleQueryContextRequests({code: 400, name: 'NotSupported', message: 'idPattern usage in query'});
        return;
    }

    contextEntity.id = req.body.entities[0].id;
    contextEntity.type = req.body.entities[0].type;
    var queryAtts = req.body.attrs;
