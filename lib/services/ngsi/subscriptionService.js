...
function createSubscriptionHandlerNgsi2(device, triggers, store, callback) {
    return function(error, response, body) {
        console.log('createSubscriptionHandlerNgsi2');
        if (error) {
            logger.debug(
                context,
                'Transport error found subscribing device with id [%s] to entity [%s]', device.id, device.name);
...
function createSubscriptionHandler(device, triggers, store, callback) {
    if (config.checkNgsi2()) {
                console.log('creando con ngsiv2');

        createSubscriptionHandlerNgsi2(device, triggers, store, callback);
    } else {
        createSubscriptionHandlerNgsi1(device, triggers, store, callback);
    }
}

/**
 * Makes a subscription for the given device's entity using NGSIv1, triggered by the given attributes. The contents of the
...
/**
 * Makes a subscription for the given device's entity using NGSIv2, triggered by the given attributes. The contents of the
 * notification can be selected using the "content" array (that can be left blank to notify the complete entity).
 *
 * @param {Object} device       Object containing all the information about a particular device.
 * @param {Object} triggers     Array with the names of the attributes that would trigger the subscription
...
function subscribeNgsi2(device, triggers, content, callback) {
    var options = {
        ...
        json: {
                ...
                expires: config.getConfig().deviceRegistrationDuration,
...
/**
 * Generate a new unsubscription request handler using NGSIv1, based on the device and subscription ID given to the function.
 *
 * @param {Object} device       Object containing all the information about a particular device.
 ...
 * @return {Function}           Returns a request handler for the given data.
 */
function createUnsuscribeHandlerNgsiv1(device, id, callback) {
    return function(error, response, body) {
        if (error || !body) {
            logger.debug(
                context,
...
/**
 * Generate a new unsubscription request handler using NGSIv2, based on the device and subscription ID given to the function.
 *
 * @param {Object} device       Object containing all the information about a particular device.
 * @param {String} id           ID of the subscription to remove.
 * @param {Function} callback   Callback to be called when the subscription handler ends.
 * @return {Function}           Returns a request handler for the given data.
 */
function createUnsuscribeHandlerNgsiv2(device, id, callback) {
    return function(error, response, body) {
        if (error) {
            logger.debug(
                context,
...
function createUnsuscribeHandler(device, id, callback) {
    if (config.checkNgsi2()) {
        createUnsuscribeHandlerNgsi2(device, id, callback);
    } else {
        createUnsuscribeHandlerNgsi1(device, id, callback);
    }
}
 ...
 * @param {String} id           ID of the subscription to remove.
 */
function unsubscribeNgsiv1(device, id, callback) {
    var options = {
        method: 'POST',
        headers: {
            'fiware-service': device.service,
 ...
 * @param {String} id           ID of the subscription to remove.
 */
function unsubscribeNgsiv2(device, id, callback) {
    var options = {
        method: 'DELETE',
        headers: {
            'fiware-service': device.service,
